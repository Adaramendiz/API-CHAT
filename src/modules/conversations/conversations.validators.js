...
const createConversationValidator = [
  check("title", "Error con el título de la conversación")
    ...
    .isLength({ max: 30 })
    .withMessage("El título no debe superar los 30 caracteres"),
...
const createGroupConversationValidator = [
  ...
  check("participants", "Error con los participantes del grupo")
    .exists()
    .withMessage("La propiedad 'participants' no debe estar vacía")
    .isArray()
    .withMessage("La propiedad 'participants' debe ser un array")
    .custom((participants) => {
